...
class WeekEditorState extends MusicBeatState
{
	...
	function reloadBG() {
		...
		if(assetName != null && assetName.length > 0) {
			if(Assets.exists(Paths.image('menus/storymode/menubackgrounds/menu_' + assetName), IMAGE)) {
	...
	function reloadWeekThing() {
		...
		var isMissing:Bool = true;
		if(assetName != null && assetName.length > 0) {
			if(Assets.exists(Paths.image('menus/storymode/weeks/' + assetName), IMAGE)) {
				weekThing.loadGraphic(Paths.image('menus/storymode/weeks/' + assetName));
				isMissing = false;
			}
		}
		...
		if(isMissing) {
			...
			missingFileText.text = 'MISSING FILE: images/menus/storymode/weeks/' + assetName + '.png';
		}
