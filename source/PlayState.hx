...
import usefulshits.DialogueBoxPsych;
import usefulshits.DialogueBox;
import usefulshits.WeekData;
import usefulshits.HScriptHandler;
import usefulshits.AttachedSprite;
import schmovin.SchmovinStandalone;
import schmovin.SchmovinAdapter;
import schmovin.PsychSchmovinAdapter;
#if sys
...
class PlayState extends MusicBeatState
{
	...
	public var realGameCam:FlxCamera;
	...
	// lifted from Funkin-Multikey once again lel
	var scripts:Array<HScriptHandler> = [];
	var script:HScriptHandler;

	// Schmoving
	public static var schmove:SchmovinStandalone;

	override public function create()
	{
		SchmovinAdapter.setInstance(new PsychSchmovinAdapter());
		schmove = new SchmovinStandalone();
		...
		schmove.afterCameras(camGame, camHUD);
		FlxG.cameras.add(camHUD);
		FlxG.cameras.add(camOther);

		@:privateAccess
		realGameCam = schmove.instance.camGame;
	...
	#if HSCRIPT_ALLOWED
	var file:String = Paths.hx("data/" + Paths.formatToSongPath(PlayState.SONG.song) + "/script");
	trace(file);
	...
	{
	#else
	trace(OpenFlAssets.exists(file));

	if (OpenFlAssets.exists(file))
	{
	#end
		trace("ADDING SCRIPT: " + file);
		script = new HScriptHandler(file);
		script.start();
		...
		timeTxt = new FlxText(STRUM_X + (FlxG.width / 2) - 248, 24, 400, "", 16);
		...
		timeBar.createFilledBar(0xFF333333, FlxColor.LIME);
		...
		scoreTxt = new FlxText(healthBarBG.x + healthBarBG.width - 190, healthBarBG.y + 30, FlxG.width, "", 16);
		scoreTxt.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		scoreTxt.scrollFactor.set();
		scoreTxt.borderSize = 1;
		scoreTxt.scale.set(1.0001, 1.0001);
		scoreTxt.visible = !ClientPrefs.hideHud;
		add(scoreTxt);

		ratingTxt = new FlxText(healthBarBG.x + 95, healthBarBG.y + 30, FlxG.width, "", 16);
		...
		ratingTxt.scale.set(1.0001, 1.0001);
		ratingTxt.visible = !ClientPrefs.hideHud;
		add(ratingTxt);

		botplayTxt = new FlxText(400, healthBarBG.y - 40, FlxG.width - 800, "AUTOPLAY", 32);
		...
		scriptsCall("createPost", []);
	}

	function scriptsCall(func:String, ?args:Array<Dynamic>)
	{
		#if HSCRIPT_ALLOWED
	...
	{
		schmove.preDraw(this);
		super.draw();
		schmove.postDraw(this);
	}
	...
	override public function update(elapsed:Float)
	{
		...
		if (ratingString == '?')
		{
			ratingTxt.text = 'Rating: ' + ratingString;
		}
		else
		{
			ratingTxt.text = 'Rating: ' + (Math.floor(ratingPercent * 10000) / 100) + '% | ' + ratingString;
		}
			...
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if (daNote.copyY)
				{
					if (ClientPrefs.downScroll)
					{
						...
						schmove.postNotePosition(this, strumLine, daNote, SONG);
				...
				if (!daNote.mustPress && daNote.wasGoodHit && !daNote.hitByOpponent && !daNote.ignoreNote)
				{
					...
					scriptsCall("enemyNoteHit", [daNote]);
				}
		...
		scriptsCall("updatePost", [elapsed]);
	}
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String)
	{
		...
		stage.triggerEventNote(eventName, value1, value2);
	}
		...
		if (!transitioning)
		{
			...
			if (isStoryMode)
			{
				...
				else
				{
					...
					schmove.onExitPlayState(new PlayState());
				}
			}
			else
			{
				...
				changedDifficulty = false;
				cpuControlled = false;
				schmove.onExitPlayState(new FreeplayState());
			}
			transitioning = true;
		}
	}
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		var pixelShitPart1:String = "ui/ratings/";
		var pixelShitPart2:String = '';

		if (PlayState.curUiType == 'pixel')
		{
			pixelShitPart1 = 'ui/pixelUI/ratings/';
